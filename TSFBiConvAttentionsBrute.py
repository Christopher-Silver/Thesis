...
spreadsheet_path = '/home/crsilver/scratch/TSF_Dataset/TSF.xlsx'
...
train_cache_file = '/home/crsilver/scratch/train_cache_TSF.npz'
val_cache_file = '/home/crsilver/scratch/val_cache_TSF.npz'
...
def sort_frames_numerically(filenames):
    # Extract the last numeric sequence from the filename for sorting
    def extract_number(f):
        s = re.findall(r"(\d+)(?=\.\w+$)", f)  # Find number before the file extension
        return int(s[-1]) if s else -1  # Use the last number found
...
class CachedNumpyDataGenerator:
    ...
    def __next__(self):
        ...
        while len(X_batch) < self.batch_size:
            ...
            if 'ADL' in video_name:
                start_min = 0
                start_max = max(0, totalFrames - (self.num_frames*3))
            else:
                #if self.num_frames > framesAfterFall:
                    #raise ValueError("numFrames must be less than framesAfterFall")

                start_min = max(0, firstFallFrameOfVideo - (self.num_frames*3))
                start_max = min(firstFallFrameOfVideo - ((self.num_frames*3) // 2), totalFrames - (self.num_frames*3))
            ...
            frames_array = [
                self.cache[os.path.join(frames_dir, frames[j])]
                for j in range(start_frame, start_frame + self.num_frames * 3, 3)
            ]
...
from tensorflow.keras.layers import ConvLSTM2D, Concatenate
...
x = FeatureBasedAttention(reduction_ratio=32)(x)
...
checkpoint = ModelCheckpoint(f'model_checkpoint_TSF2.keras', save_best_only=True, monitor='val_loss', mode='min')
